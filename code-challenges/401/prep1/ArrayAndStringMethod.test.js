...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((item) => {
    let indexx = Number(item.indexOf(' ', 3)) + 1;
    result.push(item.slice(indexx, item.length));
  });
  return result;
};
...
const stepActions = (recipe) => {
  ...
  let indexx = item.indexOf(" ")
  let action = item.slice(0, indexx)
  result.push(action)
  })
  return result;
};
...
const removeEvenValues = (arr) => {
  ...
  for(let i = evenIndex.length-1 ; i >=0 ; i-- ){
    arr.splice(evenIndex[i], 1)
  }
...
const extractVowels = (str) => {
  ...
  let finalWord = arrWord.join('')
  let finalVowels = arrVowels.join('')
  return [finalWord, finalVowels]
};
