...
function getCurrentEvents(request, response) {
    // Solution code here...
    let getCurrentEvents = mapCurrentEvents();
    response.status(200).send(getCurrentEvents);
}

const mapCurrentEvents = () => {
    // Solution code here...
    return currentEvents.news.map((data) => {
...
const calculateAverage = (arr) => {
    // Solution code here...
    let obj = arr.reduce((avg, item) => {
        avg.count += 1
        avg.sum += item
        return avg
    }, { count: 0, sum: 0 })
    let answer = obj.sum / obj.count
    return answer
};
...
const extractChildren = (arr) => {
    ...
    let newArray = arr.filter(item => {
        let regex = /a/g;
        if (regex.test(item.name) && item.children != undefined) {
            return item
        }
    })
    return newArray.reduce((childrenArray, item) => {
