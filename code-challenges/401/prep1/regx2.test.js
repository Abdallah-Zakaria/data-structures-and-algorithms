...
const validatePin = (pin) => {
    // Solution code here...
    let regex = /[0-9]/g
    return (regex.test(pin) && pin.toString().length == 4)
};
...
const validateEmail = (email) => {
    ...
    let filterArray = []
    if(emailName.split(".").length <= 2  && splitEmail.length ==2){
      filterArray = emailName.split(".").filter(item=>{
