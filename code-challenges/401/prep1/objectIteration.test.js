...
const templatingWithMustache = () => {
  ...
  characters.forEach((item) => {
    ...
    let render = Mustache.render(template, item);
    array.push(render);
  })
...
const getCourseKeys = (obj) => {
  // Solution code here...
  let arrOfObject = Object.keys(obj)
  return arrOfObject
};
...
const totalCharacters = (arr) => {
  ...
  let finalArray = []
  arr.forEach(item => {
    let tepArray = Object.values(item)
...
const houseSize = (arr) => {
  ...
  arr.forEach(houseData => {
    let obj = {}
    obj.house = houseData.house
    let numberInHouse = 1
    if (houseData.spouse != null) {
      let numberInSpouse = 1
      let numberOfChildren = houseData.children.length
      numberInHouse = numberInHouse + numberInSpouse + numberOfChildren
    }
    obj.members = numberInHouse
    sizes.push(obj)
  })
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(houseData=>{
    ...
    if(houseData.spouse != null){
      let numberOfChildren = houseData.children.length 
      numberInHouse = numberInHouse  + numberOfChildren
    }
    obj.members = numberInHouse
    survivors.push(obj)
  })
